...
local function CreateCamera()
	...
	local function OnMouseMoved(input, processed)
		...
		local mod=(game.Workspace.CurrentCamera.FieldOfView/75)
		local modx=1
		local mody=1
		local unit=false
		if LocalPlayer and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Charging") and LocalPlayer.Character:FindFirstChild("Tide Turner") == nil then
			modx=modx*1.5
			unit=true
		end
		if unit==true then
			inputDelta=Vector2.new(math.clamp(inputDelta.X,-1,1),input.Delta.Y)
		end
		if startPos and lastPos and panBeginLook then
			local currPos = lastPos + input.Delta
			local totalTrans = currPos - startPos
			if this.PanEnabled and delays==false then
				local desiredXYVector = this:MouseTranslationToAngle(inputDelta) 
				desiredXYVector=desiredXYVector* Vector2.new(MOUSE_SENSITIVITY.X*mod*modx,MOUSE_SENSITIVITY.Y*mod*mody)
				this.RotateInput = this.RotateInput + desiredXYVector
			end
			lastPos = currPos
	...
	local numOfSeconds = 0.25
	local currentSpeed = 0
	local maxSpeed = 4
	local vrMaxSpeed = 4
	local lastThumbstickPos = Vector2.new(0,0)
	local ySensitivity = 0.5
	...
	game.Workspace.CurrentCamera:GetPropertyChangedSignal("FieldOfView"):Connect(function() ----print("yes")
