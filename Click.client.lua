...
local clicked = script.Parent:FindFirstChild("Clicked")
local player = game.Players.LocalPlayer
local name, num = string.gsub(script.Parent.Name, "Button", "")

local first = script.Parent.Parent.Parent:WaitForChild("ROBLOXians")
local second = script.Parent.Parent.Parent:WaitForChild("MissingInAction")
local third = script.Parent.Parent.Parent:WaitForChild("ConfirmedDead")
local fourth = script.Parent.Parent.Parent:WaitForChild("Spectators")
...
function updateScroll()
	...
	for j, k in pairs (script.Parent.Parent:GetChildren()) do
		if k:IsA("TextButton") then
			for l, m in pairs (script.Parent.Parent:GetChildren()) do
	...
	for i, frame in pairs(script.Parent.Parent.Parent:GetChildren()) do
	...
	script.Parent.Parent.Parent.CanvasSize = UDim2.new(0, 0, (0.06 * numFramesVisible), 0)
	...
	if numFramesVisible < 11 then
		...
		script.Parent.Parent.Parent.CanvasPosition = Vector2.new(0, 0)
	...
	script.Parent.Parent.Parent.Parent.Position = UDim2.new(0.5, -background.Size.X.Offset/2, 0.5, -background.Size.Y.Offset/2)
end
...
script.Parent.MouseButton1Click:connect(function()
	...
	if corresponding.Visible == true  then
		if corresponding:FindFirstChild("PlayerName").Text ~= player.Name then
			if not clicked.Value then
				player.PlayerGui.Sounds.Click:Play()
				clicked.Value = true
				--script.Parent.Size = UDim2.new(0, 880, 0, 50)
				corresponding.Size = UDim2.new(0, 580, 0, 50)
				for i, v in pairs (corresponding:GetChildren()) do
				...
				clicked.Value = false
				--script.Parent.Size = UDim2.new(0, 880, 0, 25)
				corresponding.Size = UDim2.new(0, 580, 0, 25)
				for i, v in pairs (corresponding:GetChildren()) do
